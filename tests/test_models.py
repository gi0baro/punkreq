import gzip

import pytest

import punkreq
from punkreq import Request, Response
from punkreq._content import AsyncByteStream, AsyncIteratorByteStream


def stream_of(*chunks):
    async def gen():
        for chunk in chunks:
            yield chunk

    return AsyncIteratorByteStream(gen())


class TrackingStream(AsyncByteStream):
    """Two chunks; records whether its iterator was finalized and how often it was closed."""

    def __init__(self):
        self.iter_finalized = False
        self.close_calls = 0

    def __aiter__(self):
        return self._iterate()

    async def _iterate(self):
        try:
            yield b"one"
            yield b"two"
        finally:
            self.iter_finalized = True

    async def close(self):
        self.close_calls += 1


# ----- Request -----


def test_request_basic():
    request = Request("get", "https://example.com/path")
    assert request.method == "GET"
    assert request.url == "https://example.com/path"
    assert request.headers["host"] == "example.com"
    assert request.content == b""


def test_request_timeout_field():
    assert Request("GET", "https://example.com/").timeout is None
    request = Request("GET", "https://example.com/", timeout=3.0)
    assert request.timeout == punkreq.Timeout(3.0)  # normalized to Timeout


def test_request_host_header_with_port_and_ipv6():
    assert Request("GET", "https://example.com:8443/").headers["host"] == "example.com:8443"
    assert Request("GET", "http://[::1]:8080/").headers["host"] == "[::1]:8080"


def test_request_host_not_overridden():
    request = Request("GET", "https://example.com/", headers={"Host": "override.example"})
    assert request.headers["host"] == "override.example"


def test_request_params_merged_into_url():
    request = Request("GET", "https://example.com/?a=1", params={"b": 2})
    assert str(request.url) == "https://example.com/?a=1&b=2"


def test_request_content_length_zero_for_bodyless_post():
    assert Request("POST", "https://example.com/").headers["content-length"] == "0"
    assert "content-length" not in Request("GET", "https://example.com/").headers


def test_request_json_body():
    request = Request("POST", "https://example.com/", json={"a": 1})
    assert request.headers["content-type"] == "application/json"
    assert request.content == b'{"a":1}'
    assert request.headers["content-length"] == "7"


def test_request_explicit_content_type_wins():
    request = Request(
        "POST", "https://example.com/", json={"a": 1}, headers={"content-type": "application/vnd.custom+json"}
    )
    assert request.headers["content-type"] == "application/vnd.custom+json"


@pytest.mark.asyncio
async def test_request_streaming_body():
    request = Request("POST", "https://example.com/", content=iter([b"a", b"b"]))
    assert request.headers["transfer-encoding"] == "chunked"
    with pytest.raises(punkreq.RequestNotRead):
        request.content
    assert await request.read() == b"ab"
    assert request.content == b"ab"
    # after read() the body is repackaged and replayable
    assert await request.read() == b"ab"


def test_request_repr():
    assert repr(Request("GET", "https://example.com/")) == "<Request('GET', 'https://example.com/')>"


# ----- Response: content -----


@pytest.mark.asyncio
async def test_response_from_content():
    response = Response(200, content=b"body", request=Request("GET", "https://example.com/"))
    assert response.status_code == 200
    assert response.reason_phrase == "OK"
    assert await response.read() == b"body"
    assert await response.text() == "body"
    assert response.headers["content-length"] == "4"
    assert response.is_closed
    assert repr(response) == "<Response [200 OK]>"


@pytest.mark.asyncio
async def test_response_from_text():
    response = Response(200, text="héllo")
    assert response.headers["content-type"] == "text/plain; charset=utf-8"
    assert await response.read() == "héllo".encode()


@pytest.mark.asyncio
async def test_response_from_json():
    response = Response(200, json={"a": 1})
    assert response.headers["content-type"] == "application/json"
    assert await response.json() == {"a": 1}


@pytest.mark.asyncio
async def test_response_charset_from_content_type():
    response = Response(
        200, content="olá".encode("iso-8859-1"), headers={"content-type": "text/plain; charset=iso-8859-1"}
    )
    assert response.encoding == "iso-8859-1"
    assert await response.text() == "olá"


def test_response_unknown_charset_falls_back():
    response = Response(200, content=b"x", headers={"content-type": "text/plain; charset=zebra"})
    assert response.encoding == "utf-8"


@pytest.mark.asyncio
async def test_response_callable_default_encoding():
    response = Response(200, content="olá".encode("iso-8859-1"), default_encoding=lambda content: "iso-8859-1")
    assert await response.text() == "olá"


@pytest.mark.asyncio
async def test_response_explicit_encoding_setter():
    response = Response(200, content="olá".encode("iso-8859-1"))
    response.encoding = "iso-8859-1"
    assert await response.text() == "olá"
    with pytest.raises(ValueError):
        response.encoding = "utf-8"


def test_response_status_predicates():
    assert Response(102).is_informational
    assert Response(200).is_success
    assert Response(302).is_redirect
    assert Response(404).is_client_error
    assert Response(503).is_server_error
    assert Response(404).is_error
    assert Response(301, headers={"location": "/x"}).has_redirect_location
    assert not Response(301).has_redirect_location


def test_response_raise_for_status():
    request = Request("GET", "https://example.com/")
    assert Response(200, request=request).raise_for_status() is not None
    # reqwest semantics: 3xx does not raise
    assert Response(302, request=request, headers={"location": "/x"}).raise_for_status() is not None
    with pytest.raises(punkreq.HTTPStatusError) as exc_info:
        Response(404, request=request).raise_for_status()
    assert exc_info.value.response.status_code == 404
    assert exc_info.value.request is request
    assert "404 Not Found" in str(exc_info.value)
    with pytest.raises(punkreq.HTTPStatusError):
        Response(500, request=request).raise_for_status()


def test_response_links():
    response = Response(
        200,
        headers={"link": '<https://example.com/next>; rel="next", <https://example.com/last>; rel="last"'},
    )
    assert response.links["next"]["url"] == "https://example.com/next"
    assert response.links["last"]["url"] == "https://example.com/last"
    assert Response(200).links == {}


def test_response_elapsed_unset_raises():
    with pytest.raises(RuntimeError):
        Response(200).elapsed


def test_response_cookies_parsed_from_set_cookie():
    request = Request("GET", "https://example.com/")
    response = Response(
        200,
        headers=[("set-cookie", "session=abc; Path=/"), ("set-cookie", "theme=dark; Path=/")],
        request=request,
    )
    assert response.cookies["session"] == "abc"
    assert response.cookies["theme"] == "dark"
    assert Response(200, request=request).cookies.get("nope") is None


def test_response_history_default():
    assert Response(200).history == []


# ----- Response: streaming -----


@pytest.mark.asyncio
async def test_response_read_is_cached():
    response = Response(200, stream=stream_of(b"he", b"llo"))
    assert await response.read() == b"hello"
    assert await response.read() == b"hello"  # idempotent, cached
    assert response.is_closed


@pytest.mark.asyncio
async def test_response_iter_bytes_decodes_gzip():
    payload = gzip.compress(b"hello world")
    response = Response(200, headers={"content-encoding": "gzip"}, stream=stream_of(payload))
    assert b"".join([chunk async for chunk in response.iter_bytes()]) == b"hello world"
    assert response.num_bytes_downloaded == len(payload)


@pytest.mark.asyncio
async def test_response_iter_raw_undecoded():
    payload = gzip.compress(b"hello world")
    response = Response(200, headers={"content-encoding": "gzip"}, stream=stream_of(payload))
    assert b"".join([chunk async for chunk in response.iter_raw()]) == payload


@pytest.mark.asyncio
async def test_response_iter_raw_aborted_closes_stream_iterator():
    # early exit must aclose the stream's iterator deterministically
    # (`async for` alone abandons it to GC) and close the response
    stream = TrackingStream()
    response = Response(200, stream=stream)
    raw = response.iter_raw()
    assert await raw.__anext__() == b"one"
    await raw.aclose()
    assert stream.iter_finalized
    assert stream.close_calls == 1
    assert response.is_closed


@pytest.mark.asyncio
async def test_response_close_closes_abandoned_iterator():
    # `async for ... break` abandons its generator (the language never
    # closes it); Response.close() must aclose handed-out iterators
    # deterministically instead of leaving them to GC
    stream = TrackingStream()
    response = Response(200, stream=stream)
    body = response.iter_bytes()
    assert await body.__anext__() == b"one"
    # no aclose: simulate `async for ... break` abandonment
    await response.close()
    assert stream.iter_finalized
    assert stream.close_calls == 1
    assert response.is_closed


@pytest.mark.asyncio
async def test_response_stream_consumed_twice_raises():
    response = Response(200, stream=stream_of(b"x"))
    await response.read()
    with pytest.raises(punkreq.StreamConsumed):
        async for _ in response.iter_raw():
            pass


@pytest.mark.asyncio
async def test_response_iter_bytes_rechunks():
    response = Response(200, stream=stream_of(b"abcdefg"))
    assert [chunk async for chunk in response.iter_bytes(chunk_size=3)] == [b"abc", b"def", b"g"]


@pytest.mark.asyncio
async def test_response_iter_bytes_from_cached_content():
    response = Response(200, content=b"abcdef")
    assert [chunk async for chunk in response.iter_bytes(chunk_size=4)] == [b"abcd", b"ef"]


@pytest.mark.asyncio
async def test_response_iter_lines():
    response = Response(200, stream=stream_of(b"line1\nli", b"ne2\r\nline3"))
    assert [line async for line in response.iter_lines()] == ["line1", "line2", "line3"]


@pytest.mark.asyncio
async def test_response_iter_text():
    response = Response(200, stream=stream_of("hé".encode(), b"llo"))
    assert "".join([chunk async for chunk in response.iter_text()]) == "héllo"


@pytest.mark.asyncio
async def test_response_unknown_content_encoding_passes_through():
    response = Response(200, headers={"content-encoding": "myzip"}, stream=stream_of(b"raw"))
    assert await response.read() == b"raw"
